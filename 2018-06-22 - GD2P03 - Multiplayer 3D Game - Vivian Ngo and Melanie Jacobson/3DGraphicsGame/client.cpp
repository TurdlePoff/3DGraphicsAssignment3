...
// Description	: client implementation file. Handles all client side processes.
// Author		: Vivian Ngo
...
#pragma endregion _GETSERVER_
	...
	//std::string ip = m_pClientSocket->GetLocalAddress() );
...
bool CClient::BroadcastForServers()
{
	...
	TPacket _packet;
	_packet.Serialize(BROADCAST, "> Knock Knock");
...
void CClient::ReceiveData(char* _pcBufferToReceiveData)
{
	...
	while(m_bOnline)
	{
		...
		{
			int _iError = WSAGetLastError();
			if (_iError == WSAECONNRESET)	//WSAETIMEDOUT
			{
			...
			//ErrorRoutines::PrintWSAErrorInfo(WSAGetLastError());
			_pcBufferToReceiveData = 0;
		}
		else if (_iNumOfBytesReceived == 0)
		{
...
void CClient::ProcessData(char* _pcDataReceived)
{
	...
	case KEEPALIVE:
	{
		...
		_packet.Serialize(KEEPALIVE, "I am alive");
		SendData(_packet.PacketData);
		break;
	}
	case DATA:
